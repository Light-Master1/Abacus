 ...
 * @(#)abacus.js
 *
 * Copyright 2015 - 2022  David A. Bagley, bagleyd AT verizon.net
 *
 * All rights reserved.
 *
...
var en = {
    ...
    "Enter_calc": "Enter calculation X+Y, X-Y, or X*Y where X and result nonnegative.",
    ...
    "Answer": "Answer",
    "A-O_field_value": "A-0 field value",
    "borrow": "borrow",
    "borrowing": "borrowing",
    "Borrowing": "Borrowing",
    "carry": "carry",
    "carrying": "carrying",
    "Carrying": "Carrying",
    "Carrying_in_right_A-O_field": "Carrying in right A-O field",
    "cube": "cube",
    "Cube": "Cube",
    ...
    "multiplication_order": "mulitiplication order",
    ...
    "Subtracting": "Subtracting",
    "take_off": "take off",
    /* Next term is me being silly, abacus cannot really do negatives well. */
...
var fr = {
    "Add": "Ajouter",
    "Adding": "Ajoutant",
    "Current_answer": "Réponse actuelle",
...
function convertFromDecimal(base, x, fraction, decimalPosition) {
    ...
    while (i < string.length) {
        ...
        if (localPeriod && fraction) {
            ...
            l++;
            try {
                for (placesBase = 1; placesBase <= fractDigits; placesBase++) {
                    digit = div(number, factor);
                    outputString = outputString.concat(
                        digitToChar(digit));
                    l++;
...
function divSteps(aString, bString, decimalPosition) {
...
function multDivSteps(aString, bString) {
    /* decimal included */
    return divSteps(aString, bString, this.decimalPosition) *
        (bString.length - 1);
}

/* Number of subtraction steps for each multiplication in division */
function subMultDivSteps(aString, bString) {
    /* 2 digits per multiplication */
    return 2 * multDivSteps(aString, bString);
}

/* Finds head digits for division */
function headDividend(string, len) {
    var newString;
    ...
    return convertToDecimal(this.base, newString);
}

/* Finds head digits for division */
/* Probably will not keep since could overflow if not BigDecimal */
function headDivisor(string) {
    if (string.length == 0)
        return 0.0;
    return convertToDecimal(this.base, string);
}

/* Divide string into groups, size of group depends on root */
function rootGroup(string, root, group) {
    var decimal = getDecimalStringPosition(string);
    var length = string.length;
    ...
    // printf("rootGroup: nIntegral %d, nDecimal %d\n",
    //            nIntegral, nDecimal);
    n = nIntegral + nDecimal + 1;
    if (group != "")
        this.group = Array(n + 1);
    for (i = 0; i < nIntegral; i++) {
        ...
        for (j = 0; j < root; j++) {
            if (k - j >= 0)
                group[i] += b * letterToDigit(string[k - j]);
            else
                break;
            b *= this.base;
        }
    ...
    for (var i = 0; i < number; i++) {
        abaci[i] = {
            ...
            digit: function(display, number, place) {
                ...
                temp = v;
                for (d = (display.base >> 2) + 2; d <= (display.base >> 1) - 1; d++) {
                    temp = i + temp;
                    if (number === d)
                        return temp;
                }
                temp = v;
                for (d = (display.base >> 1); d <= (display.base >> 1) + (display.base >> 2) + 1; d++) {
                    if (number === d)
                        return temp;
                    temp = temp + i;
                }
                ...
                temp = x;
                for (d = (display.base >> 1) + (display.base >> 2) + 2; d <= display.base - 1; d++) {
                    temp = i + temp;
                    if (number === d)
                        return temp;
                }
            ...
            convertRomanFactor: function(deck) {
                var offset = (SUBBASE_TWELFTHS - this.subbase) / 4;
            ...
            shiftRails: function(oldRail, newRail) {
                ...
                if (newRail > oldRail) {
                    ...
                    if (this.checkPiece() || this.checkSubdeck()) {
                        ...
                        offset = ((this.checkPiecePercent()) ? 1 : 0);
                        for (deck = 0; deck < this.decks; deck++) {
                            for (rail = oldRail + jump - 1; rail >= newRail + jump - 1; rail--) {
                                this.beadPosition[deck][rail + offset] =
                                    this.beadPosition[deck][rail + offset - jump];
                            }
                        }
            ...
            */
                        ...
                        if (rail === this.decimalPosition +
                            ...
                            if (deck === 0) {
                                for (var subdeck = 0; subdeck < this.subdecks; subdeck++) {
                                    ...
                                    position = this.subbeadPosition[subdeck] +
                                        specialOffset;
                                    for (cell = specialOffset; cell < position; cell++) {
                                        curBead = new Bead(deck, rail, cell, cell);
                                        this.drawBead(curBead, false, false);
                                    }
                                    for (cell = position + SUBDECK_SPACES; cell < specialOffset +
            ...
            nextPositionProduct: function() {
                ...
                expandString(this.teachRString, rPlace);
                ...
                if (this.teachStep == 0) {
                    ...
                    switch (this.teachOp) {
                        ...
                        case '/':
                            ...
                            this.teachBString = decimalSafe(this.teachBString);
                            this.teachRString = this.teachBString;
                            this.teachSString = "0.0";
                            this.setText(translator.getStr("Current_answer") +
                                ": " + this.teachSString, 2);
                            this.teachStep++;
                            this.reg = 0;
                        ...
                        case 'v':
                        case 'u':
                            /* this.group = new Array(this.intGroup * 11);*/
                            this.group = "";
                            /*rootGroup(this.teachAString, (this.teachOp) ? 2 : 3, this.intGroup);
                            stringGroup(this.intGroup, group, this.teachOp);*/
                            this.convertStringToAbacus(this.teachAString, newAux, '.');
                            this.clearAux(1);
                            this.clearAux(2);
                            a = convertToDecimal(base, this.teachAString, '.', this.decimalPosition);
                            ...
                            this.setText(translator.getStr("Current_answer") +
                                ": " + this.teachSString, 2);
                            this.teachStep++;
                            this.teachBString = "0.0";
                            this.teachCString = "0.0";
                            this.reg = 0; /* LEE_ROOT -1 */
                            this.setText(translator.getStr("Current_answer") +
                                ": TBD", 2);
                            this.teachStep = 0;
                            return;
                    ...
                    switch (this.teachOp) {
                        ...
                        case '*':
                            ...
                            if (this.teachCarryStep === 0) {
                                ...
                                buf = convertFromDecimal(this.base,
                                    this.teachBDigit, false, this.decimalPosition);
                                buf = trimString(buf);
                                buffer1 = buffer1.concat(" " + this.teachOp + " " + buf);
                                buf = convertFromDecimal(this.base,
                                    (this.teachADigit * this.teachBDigit), false, this.decimalPosition);
                                buf = trimString(buf);
                                buffer1 = buffer1.concat(" = " + buf);
                                buf = convertFromDecimal(this.base,
                        ...
                        case '/':
                            ...
                            buffer1 = translator.getStr("Multiplying") +
                                ...
                                " " + this.teachBString;
                            if (this.teachCarryStep === 0 && this.divisor != 0) {
                                if (this.reg >= 0) {
                                    var bn;
                                    var qrPosition;
                                }
